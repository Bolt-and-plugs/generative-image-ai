...
class GAN(ABC):
    ...
    def __init__(
        ...
        # LPIPS model for perceptual similarity (used in evaluation and/or augmentation)
        ...
        self.save_best_model_metric = (
            self.metrics_to_calculate[0]
            if self.metrics_to_calculate
            else AvailableMetrics.FID
        )
    ...
    def save_best_model(self, metrics: Metrics, epoch: int) -> bool:
        ...
        if self.save_best_model_metric == AvailableMetrics.FID:
            ...
            if fid < self._best_metric_value:
                ...
                print(
                    f"[{self.__class__.__name__}] ✓ Novo melhor modelo na época {epoch} "
                    f"(FID={fid:.4f}) → salvo em {self.output_path}/generator_best.pth"
                )
                return True
            ...
            if psnr > self._best_metric_value:  # Higher PSNR is better
                ...
                print(
                    f"[{self.__class__.__name__}] ✓ Novo melhor modelo na época {epoch} "
                    f"(PSNR={psnr:.4f}) → salvo em {self.output_path}/generator_best.pth"
                )
                return True
            ...
            if ssim > self._best_metric_value:  # Higher SSIM is better
                ...
                print(
                    f"[{self.__class__.__name__}] ✓ Novo melhor modelo na época {epoch} "
                    f"(SSIM={ssim:.4f}) → salvo em {self.output_path}/generator_best.pth"
                )
                return True
            ...
            if lpips < self._best_metric_value:
                ...
                print(
                    f"[{self.__class__.__name__}] ✓ Novo melhor modelo na época {epoch} "
                    f"(LPIPS={lpips:.4f}) → salvo em {self.output_path}/generator_best.pth"
                )
                return True
            ...
            if w_dist > self._best_metric_value:
                ...
                print(
                    f"[{self.__class__.__name__}] ✓ Novo melhor modelo na época {epoch} "
                    f"(W_DIST={w_dist:.4f}) → salvo em {self.output_path}/generator_best.pth"
                )
                return True
        else:
            print(
    ...
    def run(self):
        ...
        pbar = tqdm(range(num_epochs), desc=f"Treinando {self.__class__.__name__}")
        ...
        print(
            f"\n[{self.__class__.__name__}] Treinamento concluído. "
            f"Melhor Metrica={self._best_metric_value:.4f} na época {self._best_epoch}."
        )
